      ...
      <div className="bg-dnd-paper text-dnd-dark rounded-sm shadow-2xl overflow-hidden relative print:shadow-none">
        ...
        <div className="p-8 border-b-2 border-dnd-red/20 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          ...
          <div className="text-right">
            <div className="inline-block border-4 border-dnd-gold p-3 rounded-full w-24 h-24 flex flex-col items-center justify-center bg-white shadow-inner">
               <span className="text-xs uppercase font-bold text-gray-500">{t('level')}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-0">
           ...
           <div className="md:col-span-4 bg-gray-100/50 p-6 border-r border-dnd-red/10 flex flex-col gap-6">
                ...
                {(Object.keys(character.stats) as Ability[]).map((ability) => {
                  ...
                  return (
                    <div key={ability} className="flex flex-col items-center bg-white border border-gray-300 rounded-xl p-3 shadow-sm">
                        <span className="text-xs font-bold uppercase text-gray-500">{t(ability)}</span>
              ...
              {/* Skills List */}
              <div className="mt-4">
                 <h3 className="font-serif font-bold text-dnd-red uppercase border-b border-gray-300 pb-1 mb-2">{t('skills')}</h3>
                   ...
                   {SKILL_DATA.map(skill => {
                      ...
                      return (
                        <div key={skill.name} className="flex items-center text-sm">
                           ...
                           <span className="ml-auto text-[10px] uppercase text-gray-400">{t(skill.ability).substring(0, 3)}</span>
                        </div>
                      )
                   })}
                 </div>
           ...
           <div className="md:col-span-8 p-8 space-y-8">
              ...
              <div className="flex flex-wrap gap-6 justify-center mb-8">
                  ...
                  <div className="flex gap-4">
                      <div className="border-2 border-gray-300 rounded-lg p-3 text-center w-28 bg-white shadow-sm relative overflow-hidden">
                        ...
                        <span className="text-xs font-bold text-gray-400 uppercase block mb-1">{t('armorClass')}</span>
                      ...
                      <div className="border-2 border-gray-300 rounded-lg p-3 text-center w-24 bg-white shadow-sm relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-full h-1 bg-gray-400 opacity-50"></div>
                        <span className="text-xs font-bold text-gray-400 uppercase block mb-1">{t('initiative')}</span>
                        <div className="flex items-center justify-center h-10">
                      ...
                      <div className="border-2 border-gray-300 rounded-lg p-3 text-center w-24 bg-white shadow-sm relative overflow-hidden">
                        <div className="absolute top-0 left-0 w-full h-1 bg-gray-400 opacity-50"></div>
                        <span className="text-xs font-bold text-gray-400 uppercase block mb-1">{t('speed')}</span>
                        <div className="flex items-center justify-center h-10">
                  ...
                  <div className="flex gap-4">
                      <div className="border-2 border-dnd-red/30 rounded-lg p-3 w-48 bg-white shadow-sm relative overflow-hidden flex flex-col justify-between">
                         ...
                         <div className="flex justify-between items-center mb-1">
                             <span className="text-xs font-bold text-gray-400 uppercase">{t('healthPoints')}</span>
                      ...
                      <div className="border-2 border-gray-300 rounded-lg p-3 w-32 bg-white shadow-sm relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-center mb-1">
                             <span className="text-xs font-bold text-gray-400 uppercase">{t('hitDie')}</span>
              ...
              <div className="grid grid-cols-2 gap-8">
                 <div className="flex items-center gap-4 border-b border-gray-300 pb-2">
                    ...
                    <div className="text-sm font-bold uppercase text-gray-600">{t('pb')}</div>
                 </div>
                 <div className="flex items-center gap-4 border-b border-gray-300 pb-2">
                    ...
                    <div className="text-sm font-bold uppercase text-gray-600">{t('passivePerception')}</div>
                 </div>
              </div>
              ...
              {character.feats.length > 0 && (
                <div className="mt-4">
                  <h3 className="font-serif font-bold text-xl text-dnd-red border-b-2 border-dnd-red mb-4">{t('featuresTraits')}</h3>
                    ...
                    {character.feats.map((feat, idx) => (
                      <div key={idx} className="bg-gray-50 rounded p-4 border border-gray-200">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {feat.isActive && <span className="text-[10px] uppercase text-green-600 font-bold tracking-wider">• {t('active')}</span>}
                          {feat.repeatable && <span className="text-[10px] uppercase text-purple-600 font-bold tracking-wider border border-purple-200 px-1.5 rounded bg-purple-50">{t('repeatable')}</span>}
                        </div>
              ...
              {isSpellcaster && (
                <div className="mt-8">
                    <div className="flex items-center gap-3 border-b-2 border-dnd-red mb-4 pb-2">
                        <h3 className="font-serif font-bold text-xl text-dnd-red">{t('magicSpellcasting')}</h3>
                        <span className="text-xs uppercase font-bold text-gray-500 border border-gray-300 px-2 py-0.5 rounded bg-white">
                            {t(character.spellcastingAbility) || character.spellcastingAbility}
                        </span>
                    </div>

                    <div className="grid grid-cols-3 gap-4 mb-6">
                        <div className="bg-gray-50 border border-gray-200 rounded p-3 text-center">
                            <span className="text-xs uppercase font-bold text-gray-500 block">{t('spellSaveDC')}</span>
                            <span className="text-3xl font-serif font-bold text-dnd-dark">{saveDC}</span>
                        </div>
                        <div className="bg-gray-50 border border-gray-200 rounded p-3 text-center">
                            <span className="text-xs uppercase font-bold text-gray-500 block">{t('attackBonus')}</span>
                        ...
                        <div className="bg-gray-50 border border-gray-200 rounded p-3 text-center flex flex-col justify-center">
                           <span className="text-xs uppercase font-bold text-gray-500 block">Mod</span>
                            ...
                            return (
                                <div key={level}>
                                    <div className="flex justify-between items-end border-b border-gray-300 mb-2">
                                        <h4 className="font-bold text-dnd-dark">{t('level')} {level}</h4>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                                        {spells.length === 0 ? <span className="text-xs text-gray-400 italic">No spells known/prepared</span> : spells.map((spell, idx) => (
                                            <div key={idx} className="flex justify-between items-center text-sm p-2 bg-gray-50 rounded border border-gray-200">
                                                <span>{spell.name}</span>
                                                {spell.prepared && <span className="text-[10px] uppercase font-bold text-dnd-red bg-red-50 px-1 rounded border border-red-100">{t('prepared')}</span>}
                                            </div>
                                        ))}
                                    </div>
                                </div>
              ...
              {character.equipment.length > 0 && (
                <div className="mt-8">
                  <h3 className="font-serif font-bold text-xl text-dnd-red border-b-2 border-dnd-red mb-4">{t('equipment')}</h3>
              ...
              {/* Backstory Section */}
              <div className="mt-8">
                <h3 className="font-serif font-bold text-xl text-dnd-red border-b-2 border-dnd-red mb-4">{t('backstoryAppearance')}</h3>
              ...
              <div className="pt-12 flex justify-end print:hidden">
                 ...
                 >
                   <Download className="w-5 h-5" /> {t('print')}
                 </button>
              </div>
           </div>
        </div>
